...
Analyzes call/session transcripts using LiteLLM and returns
...
Environment Variables:
    LITELLM_BASE_URL: LiteLLM base URL (default: http://localhost:10006)
    MODEL_ID:         Model ID to use (default: claude-3-haiku)
...
LITELLM_BASE_URL = os.environ.get('LITELLM_BASE_URL', 'http://localhost:10006')
MODEL_ID = os.environ.get('MODEL_ID', 'claude-3-haiku')
MAX_TOKENS = int(os.environ.get('MAX_TOKENS', '2048'))
TEMPERATURE = float(os.environ.get('TEMPERATURE', '0.3'))

client = OpenAI(base_url=f"{LITELLM_BASE_URL}/v1", api_key=os.environ.get('LITELLM_API_KEY', 'no-key'))
...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(f'Error: {str(e)}')
        return error_response(f'Analysis failed: {str(e)}', 'LITELLM_ERROR', 500)
